...
zerod_baseline_res_dict[2]["pressure_in"]  = zerod_baseline_res_dict[0]["pressure_out"]
...
for i in range(len(branch_soln_dict.keys())):
    ...
    plt.scatter(branch_soln_dict[i]["dist"][-1]+shift, zerod_baseline_res_dict[i+2]["pressure_out"]/1333, marker = "o", s = 100, facecolors='none', edgecolors= colors1[i])
for i in range(len(zerod_model_res_dict.keys())):
    ...
    plt.scatter(pt1_loc, zerod_model_res_dict[i]["pressure_in"]/1333, marker = "*", s = 100, color = colors1[i])
    plt.scatter(pt2_loc, zerod_model_res_dict[i]["pressure_out"]/1333, marker = "*", s = 100, color = colors1[i]) 

plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.xlabel("Centerline distance (cm)")
plt.ylabel("Pressure (mmHg)")
plt.savefig("results/pressure_over_branches_zerod.pdf")
